    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="text-right">
          <p className="text-3xl font-bold text-white">${deal.amount.toLocaleString()}</p>
        ...
        <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl shadow-xl overflow-hidden flex flex-col">
          ...
          <div className="p-6">
            {deal.activities.length === 0 ? (
              <div className="p-12 flex flex-col items-center justify-center text-slate-400 text-sm border-2 border-dashed border-white/10 rounded-xl">
              ...
              <div className="space-y-8 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-white/10 before:to-transparent">
                {deal.activities.map((activity, index) => (
                  <div key={activity.id} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                    ...
                    {/* Card */}
                    <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-4 rounded-xl border border-white/10 bg-white/5 backdrop-blur-sm shadow-sm hover:bg-white/10 transition-colors group/card relative">
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <DeleteButton 
                            ...
                            className="opacity-0 group-hover/card:opacity-100 transition-opacity p-1 w-6 h-6 hover:bg-red-500/20 text-slate-400 hover:text-red-400" 
                          />
                        </div>
                      </div>
                      ...
                      <div className="mt-2 text-xs text-slate-500 flex items-center gap-1">
                        <span>Logged by</span>
                        <span className="text-slate-400 font-medium">{activity.user.name}</span>
                      </div>
                    </div>
                  </div>
                ))}
